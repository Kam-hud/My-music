...
export function useMusicPlayer() {
    ...
    const progressPercentage = computed(() => {
        return currentSong.value.duration ? (currentSong.value.currentTime / currentSong.value.duration) * 100 : 0
    })
    ...
    const saveSongProgress = (songId, currentTime) => {
        const progressData = JSON.parse(localStorage.getItem('songProgress') || '{}')
        progressData[songId] = {
            currentTime,
            timestamp: Date.now(),
    ...
    const addToPlayHistory = (song) => {
        ...
        playHistory.value.unshift({
            ...
            playCount: (playHistory.value.find(item => item.id === song.id)?.playCount || 0) + 1
        })
        // 限制历史记录数量
    ...
    const seekToTime = (timeInSeconds) => {
        if (audio.value.duration) {
            const validTime = Math.max(0, Math.min(timeInSeconds, audio.value.duration))
    ...
    onMounted(() => {
        ...
        if (currentPath.startsWith('/playlist/')) {
            ...
            if (!currentPlaylist.value) {
                ...
                const foundPlaylist = recommendedPlaylists.validTime.find(p => p.id == playlistId)
                if (foundPlaylist) {
        ...
        if (savedSong) {
            ...
            currentSong.value = { ...parsedSong, isPlaying: false, currentTime: 0, duration: 0 }
            if (parsedSong.url) {
        ...
        (newPath) => {
            ...
            if (!newPath.startsWith('/playlist/')) {
                ...
                if (!currentPlaylist.value) {
                    ...
                    if (foundPlaylist) {
                        currentPlaylist.value = foundPlaylist
                        localStorage.setItem('currentPlaylist', JSON.stringify(foundPlaylist))
                    }
                }
            }
        }
